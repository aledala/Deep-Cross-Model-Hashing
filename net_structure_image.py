...
MODEL_DIR = './sample/imagenet-vgg-f.mat'
def img_net_structure(input_image, bit):
	...
	for i, name in enumerate(layers[:-1]):
		...
		net[name] = current
	W_fc8 = tf.random.normal([4096, bit], stddev=1.0) * 0.01

	b_fc8 = tf.random.normal([bit],stddev = 1.0) * 0.01
